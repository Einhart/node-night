...
//[30,68,68]
            ...
            imm.inRange([98,50,50], [138,255,255]);//check BLUE hsv
            // this function will color white what it deems as blue.
          ...
          im_canny.gaussianBlur([3,3]); //gaussian blur made everything stable
          im_canny.canny(lowThresh, highThresh);


          contours = im_canny.findContours();

          //im_canny.drawAllContours(contours, WHITE);
  ...
  for (var i = 0; i < contours.size(); i++) {
    ...
    //im_canny.drawAllContours(contours, WHITE, 8);
//(solidity > 0.9) && (aspectRatio>=0.8 && aspectRatio<=1.2) && (boundingRect.width > 25 && boundingRect.height > 25)

if ((solidity > 0.99) && (aspectRatio>=0.8 && aspectRatio<=1.2)){
...
if(centerX <= 280){//if less than or equal to x=300
          drone.left(18); // wheels tend to be sticky at slow speeds.
      setTimeout(function() {
        drone.stop();
      }, 1);
                }
else if(centerX > 380){// if greater than or equal to x=360
          drone.right(18);
      setTimeout(function() {
        drone.stop();
      }, 1);
...
function closing(bx,by,bwidth,bheight){
    if(bwidth<60){
        drone.forward(38); //for forward motion 10 is sufficient. it is more robust than backwards
      setTimeout(function() {
      drone.stop();}, 1);
    }
    else if (bwidth<130){
      ...
      drone.stop();}, 1);
              ...
              drone.backward(28); // backwards movement is weaker than forwards movement
      setTimeout(function() {
      drone.stop();}, 1);
    }
    else{
